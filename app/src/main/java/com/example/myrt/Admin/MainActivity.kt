package com.example.myrt.Admin
...
import com.example.myrt.LoginActivity
import com.example.myrt.R
import com.example.myrt.UrlClass
import com.example.myrt.databinding.ActivityMainAdminBinding
import com.example.myrt.databinding.NavHeaderBinding
...
//                    startActivity(intent)
        ...
        val request = object : StringRequest(
            ...
            Response.Listener { response ->
                ...
                for (i in 0..(jsonArray.length()-1)){
                    ...
                    entries.add(Entry(x.toFloat(), index.toInt().toFloat()))
                    xvalue.add(y)
                }
        ...
        val request = object : StringRequest(
            ...
            Response.Listener { response ->
                ...
                for (i in 0..(jsonArray.length()-1)){
                    ...
                    entries1.add(Entry(y1.toFloat(), index.toInt().toFloat()))
                    entries2.add(Entry(y2.toFloat(), index.toInt().toFloat()))
                    xvalue.add(x)
                }
    ...
    private fun drawChart(entries: List<Entry>, xvalue: List<String>) {
        ...
        // Mengatur label sumbu x menggunakan IndexAxisValueFormatter
        val xAxis = b.chartLine.xAxis
    ...
    private fun drawChartKeuangan(entries1: List<Entry>, entries2: List<Entry>, xvalue: List<String>) {
        ...
        val finaldataset = ArrayList<ILineDataSet>()
        finaldataset.add(dataSet1)
        finaldataset.add(dataSet2)

        val xDataSet = LineDataSet(emptyList<Entry>(), "") // Create an empty LineDataSet for x-values
        xDataSet.color = Color.TRANSPARENT // Set color to transparent

        finaldataset.add(xDataSet) // Add xDataSet to the finaldataset

        val lineData = LineData(finaldataset)
